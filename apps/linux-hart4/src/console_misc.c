...
void set_memory_regions(void){
    #if defined( USE_HALO ) && ( USE_HALO == 1 )
        blackboard_Core3ToCore4_external_buffer = get_external_buffer(VIRTUAL_CHARGE_STATUS );
        blackboard_Core3ToCore4_external_size = CHARGE_STATUS_SIZE;
        blackboard_Core2ToCore4_external_buffer = get_external_buffer(VIRTUAL_SAFETY_STATE );
        blackboard_Core2ToCore4_external_size = SAFETY_STATE_SIZE;
    #endif
}