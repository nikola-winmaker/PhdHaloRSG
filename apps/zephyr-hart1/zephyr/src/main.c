...
/*
    ...
    */
    ...
    /* TODO HALO: 1 OK. Initialize the HALO channels for communication with the peer using init function defined in halo_api.c
    ...
    */
    halo_sensorfusion_zephyr_init_riscv64_h1_zephyr();
    ...
    /* TODO HALO: 2 OK. Declare a variable of type SensorFrame generated from HALO
      - SensorFrame struct is available from HALO generated code from deps/halo/codegen/riscv64_h1_zephyr/include/halo_structs.h
    */
    SensorFrameData stSensorFrame;
    ...
    while( 1 )
    {
        ...
        /* This is a demo loop to showcase the application running */
        /* TODO HALO: 3 OK. This is for demonstration purposes only and should be deleted when workshop code is written */
        if( ( heartbeat_counter % 20U ) == 0U)
        {
        ...
        /*TODO HALO: 4 OK. Call BMS APIs to get the sensor data and fill the SensorFrame struct with the data. 
            -- sensor_frame is a placeholder variable, it is a type of SensorFrame struct that you will define based on the workshop specification
        */
        stSensorFrame.battery_voltage_mv = (unsigned int)(bms_get_voltage());
        stSensorFrame.charge_current_ma = (int)(bms_get_current());
        stSensorFrame.battery_temp_c = bms_get_temperature();
        stSensorFrame.breaker_closed = bms_get_breaker_closed();
        stSensorFrame.fault_flags = bms_get_fault_flags();
 
        /*TODO HALO: 5 OK. Send the SensorFrame to the peer using halo_send_ API functions defined in halo_api.h
            -- Check the return value of halo_send_ function to ensure the message was sent successfully if not sent log an error message    
                printk( "[APP1] SensorFrame send failed\n" );
            -- Full path is in .deps/halo/codegen/riscv64_h1_zephyr/include/halo_api.h and deps/halo/codegen/riscv64_h1_zephyr/src/halo_channels.c
        */
        ...
        if(0 != halo_send_SensorFrameIf_SensorFrameData(&stSensorFrame))
        {
           printk( "[APP1] SensorFrame send failed\n" ); 
        }

        /*TODO HALO: 6 OK. Log the Info to the console using printk("[APP1] ") which is behaving similar to printf */
        // sensor_frame is a placeholder variable, it is a type of SensorFrame struct that you will define based on the workshop specification
         if( ( heartbeat_counter % 30 ) == 0U)
         {
             printk( "[APP1] sent mV=%u mA=%d temp=%f breaker_closed=%u faults=%d\n",
                        ( unsigned int ) stSensorFrame.battery_voltage_mv,
                        stSensorFrame.charge_current_ma,
                        stSensorFrame.battery_temp_c,
                        ( unsigned int ) stSensorFrame.breaker_closed,
                        ( unsigned int ) stSensorFrame.fault_flags );
         }
