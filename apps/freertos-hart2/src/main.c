...
static void charge_ctrl_task( void * parameters )
{
    ...
    SensorFrame sensFrame;
    ChargeCommand chCmd;
    ChargeStatus chStatus;
    ...
    halo_freertos_init_riscv64_h2_freertos();
    ...
    while( 1 )
    {
        ...
        while( halo_recv_SensorFrameIf_SensorFrame(&sensFrame) > 0)
        {
            uart_log("battery_voltage_mv: %d\ncharge_current_ma: %d\nbattery_temp_c: %d\n breaker_closed: %d\nfault_flags: %d\n",
            sensFrame.battery_voltage_mv,
            sensFrame.charge_current_ma,
            sensFrame.battery_temp_c,
            sensFrame.breaker_closed,
            sensFrame.fault_flags);
        }

        while( halo_recv_OperatorControlIf_OperatorCommand(&opCmd) > 0)
        {
            uart_log("command_id: %d\command_param: %d\n",
            opCmd.command_id,
            opCmd.command_param);
        }

        apply_operator_command(&opCmd );
        build_charge_outputs( &sensFrame, &chCmd, &chStatus );

        if(halo_send_ChargeCommandIf_ChargeCommand(&chCmd) != 0)
        {
            uart_log( "[APP2] ChargeCommand send failed\n" );
        }

        if(halo_send_ChargingStatusIf_ChargeStatus(&chStatus) != 0)
        {
            uart_log( "[APP2] ChargeStatus send failed\n" );
        }
        ...
        if( ( heartbeat_counter % 10 ) == 0U){
            uart_log( "[APP2] received mV=%d mA=%d temp=%f breaker_closed=%d faults=%d\n",
                ( uint32_t ) sensFrame.battery_voltage_mv,
                ( uint32_t ) sensFrame.charge_current_ma,
                sensFrame.battery_temp_c,
                ( uint32_t ) sensFrame.breaker_closed,
                ( uint32_t ) sensFrame.fault_flags );
        }
