    ...
    */
    ...
    while( 1 )
    {
        ...
        /*TODO Classical: 4. Receive ChargeCommand message from peer using shared memory access (read from defined memory address for ChargeCommand)
            ...
            protocol to check if new data is available before reading.
        */
       while(!charge_command.write) {
        charge_command = *((struct ChargeCommand*)CHARGE_COMMAND_BASE);
       }
       charge_command.read = true;
       charge_command.write = false;
       *((struct ChargeCommand*)CHARGE_COMMAND_BASE) = charge_command;
        ...
        struct SafetyState prev_state = safety_state;
        evaluate_safety(&charge_command, &safety_state, heartbeat_counter);
        ...
        /*TODO Classical: 6. Publish/log/send the SafetyState state to the peer using Blackboard protocol (write to defined memory address for SafetyState)
             -- Synchronization is important, so make sure to implement a simple protocol to signal when new data is available for the peer to read.
        */
       while (!safety_state.read) {
        safety_state = *((struct SafetyState*)SAFETY_STATE_BASE);
       }
       safety_state.write = true;
       safety_state.read = false;
        *((struct SafetyState*)SAFETY_STATE_BASE) = safety_state;
        ...
        if( ( heartbeat_counter % 10U ) == 0U || 
            safety_state.safe_mode != prev_state.safe_mode ||
            safety_state.breaker_open != prev_state.breaker_open ||
            safety_state.charging_allowed != prev_state.charging_allowed )
         {
            uart_log( "[APP3] CC safe_mode=%d breaker_closed=%d charging_allowed=%d heartbeat=%d\n",
                            ( uint32_t ) safety_state.safe_mode,
                            ( uint32_t ) safety_state.breaker_open ? 0 : 1, // Convert breaker_open to breaker_closed for logging
                            ( uint32_t ) safety_state.charging_allowed,
                            ( uint32_t ) safety_state.heartbeat_counter );
        }
