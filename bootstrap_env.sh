...
setup_halo_python() {
    local halo_rsg_dir="$(cd "$(dirname "$0")" && pwd)"
    local prepare_script="${halo_rsg_dir}/prepare_python.sh"

    if [ -f "${prepare_script}" ]; then
        step "Setting up HALO Python environment"
        bash "${prepare_script}"
        ok "HALO Python environment setup complete"
    else
        err "prepare_python.sh not found at ${prepare_script}"
        return 1
    fi
}
