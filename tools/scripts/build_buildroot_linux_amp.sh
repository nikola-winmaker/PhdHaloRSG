...
mkdir -p "${BOARD_DIR}/rootfs-overlay/etc/init.d"
install -m 0755 "${APP_BUILDROOT_DIR}/post-build.sh" "${BOARD_DIR}/post-build.sh"
install -m 0755 "${APP_ROOTFS_OVERLAY_DIR}/etc/init.d/S90amp-hart0-app" "${BOARD_DIR}/rootfs-overlay/etc/init.d/S90amp-hart0-app"
install -m 0644 "${APP_ROOTFS_OVERLAY_DIR}/etc/inittab" "${BOARD_DIR}/rootfs-overlay/etc/inittab"
